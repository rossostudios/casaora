    ...
    <section className="bg-black py-24 md:py-32">
      <div className="container mx-auto max-w-[1400px] px-4 md:px-8">
        <div className="mb-20 max-w-4xl">
          <motion.h2
            className="mb-6 font-medium text-4xl text-white tracking-tight md:text-5xl lg:text-6xl"
          ...
          <motion.p
            className="max-w-3xl text-lg text-white/50 md:text-xl"
            initial={{ opacity: 0, y: 20 }}
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {features.map((feature, idx) => {
            ...
            return (
              <motion.div
                className="group relative flex min-h-[450px] flex-col overflow-hidden rounded-2xl border border-white/5 bg-[#0a0a0a] transition-colors hover:bg-[#111]"
                initial={{ opacity: 0, y: 20 }}
                key={feature.title}
              ...
              >
                {/* Top Left FIG Label */}
                <div className="absolute top-6 left-6 font-mono text-[#555] text-[10px] uppercase tracking-widest">
                  {feature.fig}
                </div>

                {/* Isometric SVG Illustration */}
                <div className="flex min-h-[250px] flex-1 items-center justify-center p-8 pt-16 md:min-h-[300px]">
                  <Icon className="h-full w-full max-w-[280px] text-white/20 transition-colors duration-500 group-hover:text-white/40" />
                </div>
