    ...
    const steps = [
        {
            ...
            description: isEn ? "Basic workspace container." : "Contenedor básico del espacio.",
            done: true,
            icon: Home01Icon,
        },
        {
            ...
            description: isEn ? "Your first portfolio record." : "Tu primer registro de portafolio.",
        ...
        },
        {
            ...
            description: isEn ? "Link units to properties." : "Vincula unidades a propiedades.",
            done: unitCount > 0,
            icon: HotelIcon,
        ...
        },
        {
            ...
            description: isEn ? "Create reservations or tasks." : "Crea reservas o tareas.",
                    ...
                    {steps.map((step) => (
                        <div
                            className={cn(
                                ...
                                step.done ? "border-primary/20 opacity-75" : "hover:border-primary hover:shadow-sm"
                            )}
                            key={step.id}
                        >
                            <div className="flex items-center justify-between">
                                <div className={cn("rounded-md p-1.5", step.done ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground")}>
                                    <Icon icon={step.icon} size={18} />
                                </div>
                                {step.done ? (
                                    <Icon className="text-primary" icon={Tick02Icon} size={18} />
                                ) : (
                                    <Icon className="text-muted-foreground/30" icon={CircleIcon} size={18} />
                                )}
                            </div>
                            <div className="mt-1">
                                <p className="font-semibold text-sm leading-none">{step.label}</p>
